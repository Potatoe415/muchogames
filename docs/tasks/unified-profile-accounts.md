...
- Wired 2026-09-24: hub mints `?profileCode=` for coinchapp/tranquil; Yatzy posts `/api/profile/record-result`; both apps consume the code and increment `muchogames_profiles`; each browser folds its local counter in once. Still needs a live check once `0003_profiles.sql` is applied and Google Auth is enabled. `docs/PRODUCT.md` / `docs/ARCHITECTURE.md` / `docs/SECURITY.md` wording still needs user confirmation before editing.
