 ...
 *  on when the settings panel's debug checkbox is checked (see
 *  `HomeTopBar.tsx`, `useBataillecorseDebugMode`). Shows which game/match
 *  type is running, then every move as it happens (see
 *  `useBataillecorseDebugLog`). Deliberately English-only, always - a
 *  developer diagnostic tool, never gameplay copy, so it does not follow the
 *  app's `useI18n` locale. `pointer-events-none`: purely informational, it
 *  must never block a tap on the table underneath. */
