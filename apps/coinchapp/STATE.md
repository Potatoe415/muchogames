...
Last_Action: Added the duel mode toggle to `/local`'s setup screen (bataillecorse only), new `useLocalBataillecorseDuelGame`/`BataillecorseDuelTable`/`BataillecorseDuelCenter`, exported reusable pieces from `BataillecorseTable.tsx`. Then, per user follow-up, added a 4th home-screen button "Jouer face à face" (first, above "Jouer en local") that routes to `/local?game=bataillecorse&mode=duel`, presetting the toggle. Verified live (browser): both seats' own stock flips correctly attributed, tribute chain across seats, false-slap penalty attributed to the tapping seat with the pile flying to the other, reset/settings panel, vs-bot mode unaffected, new home button correctly presets the duel toggle. `npm test` 248/248, `npm run build`, `npm run lint` clean (same pre-existing baseline issues only).
Next_Actions:
